...
export class AlongHistorySetup extends MaterialGameSetup<PlayerColor, MaterialType, LocationType, AlongHistoryOptions> {
	...
	setupDeck() {
		this.material(MaterialType.Card).createItems(
			listingToList(AgesCards[Age.Prehistory]).map((card) => ({ id: { front: card, back: Age.Prehistory }, location: { type: LocationType.Deck } }))
		);
		this.material(MaterialType.Card).shuffle();
	}

	setupCivilisationTokens() {
		this.material(MaterialType.CivilisationToken).createItems(
			this.game.players.map((player) => ({ id: player, location: { type: LocationType.AchievementsBoard, x: 0, y: 0 } }))
		);
	}

	setupResourceItems() {
		this.material(MaterialType.Dice).createItems(
			listingToList(DiceCount)
				.filter((dice) => dice !== DiceType.Gold)
				.map((dice) => ({ id: dice, location: { type: LocationType.DiscardTile, parent: 0 } }))
		);
		this.material(MaterialType.ResultToken).createItems(
			listingToList(ResultTokens).map((id) => ({ id, location: { type: LocationType.ResultTokenStock } }))
		);
	...
	setupAchievementTokens() {
		this.material(MaterialType.AchievementToken).createItems(
			achievements
				.filter((achievement) => achievement !== Achievement.Gold15)
				.map((achievement) => ({ id: achievement, location: { type: LocationType.Table } }))
		);
		...
		for (let x = 1; x < AchievementBoardLocations.length; x++) {
			for (const y of AchievementBoardLocations[x]) {
				this.material(MaterialType.AchievementToken)
					...
					.id<Achievement>((id) => getAchievementValue(id) === x)
